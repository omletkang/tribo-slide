...
def generate_dataset(win_size=50, n_test=5): # jump
    ...
    libras_dataset = r"./tribo_260112/"
    version = "v6"
    tractory_type = "maze"

    ''' Load train set '''
    data_shape_dict = loadmat(libras_dataset + f"datacell{version}_slide_{tractory_type}.mat")
    ...
    #     raw_data_train = np.concatenate((raw_data_train, raw_data[:,i]), axis=0)

    # for i in range(nb_cell - n_test, nb_cell):
    #     raw_data_test = np.concatenate((raw_data_test, raw_data[:,i]), axis=0)

    for i in range(nb_cell):
        for j in range(nb_repeat):
            if j < nb_repeat - n_test:
                raw_data_train = np.concatenate((raw_data_train, raw_data[i,j]), axis=0)
            else:
                raw_data_test = np.concatenate((raw_data_test, raw_data[i,j]), axis=0)
...
if __name__ == "__main__":
    generate_dataset(win_size=50, n_test=2)
    # n_test = {"shape": 5, "spline": 5, "maze": 2}}
