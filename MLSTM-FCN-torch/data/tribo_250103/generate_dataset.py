...
def generate_dataset(win_size=50, n_test=5): # jump
    ...
    np.save(libras_dataset + f'X_train{win_size}.npy', X_train)
    np.save(libras_dataset + f'y_train{win_size}.npy', y_train)
    np.save(libras_dataset + f'X_test{win_size}.npy', X_test)
    np.save(libras_dataset + f'y_test{win_size}.npy', y_test)
